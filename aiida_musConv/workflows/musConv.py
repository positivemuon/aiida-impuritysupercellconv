...
@calcfunction
def re_init_supcgen(aiida_struc, aiida_SCstruc, vor_site):
...
class musConvWorkChain(WorkChain):
    ...
    def inspect_run_get_forces(self):
        ...
        if not calculation.is_finished_ok:
            self.report(
                "PwCalculation<{}> failed with exit status {}".format(
                    calculation.pk, calculation.exit_status
                )
            )
    ...
    def continue_iter(self):
        try:
            ...
            self.report(
                "Exiting muSConvWorkChain,Error in fitting the forces of supercell (iteration no. <{}>) to an exponential, maybe force data not exponential".format(
                    self.ctx.n
                )
            )
    ...
    def exit_max_iteration_exceeded(self):
        self.report(
            "Exiting muSConvWorkChain, Coverged supercell NOT achieved, next iter num <{}> is greater than max iteration number {}".format(
                self.ctx.n, self.inputs.max_iter_num.value
            )
        )
